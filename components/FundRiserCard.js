...
const FundRiserCard = ({props,pushWithdrawRequests}) => {
  ...
  return (
    ...
    <Link href={`/project-details/${props.address}`} passHref>
               <button className="p-2 w-full bg-black text-white rounded-md hover:bg-black">
...

    {
      props.state !== "Successful" ?
        <div className="w-full bg-gray-200 rounded-full">
          <div className="bg-black text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full" style={{ width: `${props.progress}%` }}> {props.progress}% </div>
        </div>
    :""
    }
